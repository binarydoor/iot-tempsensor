...
main(int argc, char *argv[])
{
  ...
  writeSize = write(socket_fd, buffer, readSize);
  if (readSize != writeSize) errorHandle(15, buffer, 2);
  //fprintf(socket_fd, "ID=%s\n", id);
  ...
  while (keepgoing) {
    ...
    if (mypoll[0].revents & POLLIN) {
      readSize = read(socket_fd, buffer, 1024);
